 ...
 * @author tim
 */
public class SequentialFileRecordReader extends RecordReader<WritableComparable, WritableComparable> {
    ...
    private WritableComparable value = null;
    private Job job;

   
    public SequentialFileRecordReader(Job job) {
        this.job = job;
    }
    ...
    @Override
    public WritableComparable getCurrentValue() {
        return value;
    }
    ...
    public void initialize(ByteBuffer input) {
        ...
        is = new DataInputStream(new ByteArrayInputStream(b));
        try {
            key = job.getMapInputKeyClass().newInstance();
            value = job.getMapInputValueClass().newInstance();
        } catch (InstantiationException ex) {
            Logger.getLogger(SequentialFileRecordReader.class.getName()).log(Level.SEVERE, null, ex);
