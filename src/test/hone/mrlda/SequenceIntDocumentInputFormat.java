 ...
 * @author tim
 */
public class SequenceIntDocumentInputFormat extends InputFormat implements Callable {
    ...
    public SequenceIntDocumentInputFormat(ByteBuffer input) {
        // make a read only copy just in case
        this.buffer = input;
        this.buffer.rewind();
    }

    public SequenceIntDocumentInputFormat() {
        // make a read only copy just in case
    }
    ...
    public RecordReader<WritableComparable, WritableComparable> createRecordReader(ByteBuffer byteBuffer) {
        RecordReader reader = new SequentialIntDocumentRecordReader(job);
