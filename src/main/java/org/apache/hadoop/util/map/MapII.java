...
public interface MapII {
  ...
  /**
   * A map entry (key-value pair) for <tt>MapII</tt>. The <tt>MapII.entrySet</tt> method returns a
   ...
   * <tt>MapII.Entry</tt> objects are valid <i>only</i> for the duration of the iteration; more
   ...
   * works properly across different implementations of the <tt>MapII</tt> interface.
   *
