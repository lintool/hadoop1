...
public class HMapIV<V> implements MapIV<V>, Cloneable, Serializable {
     ...
     */
    transient HMapIV.Entry<V>[] table;
    /**
    ...
    public HMapIV(int initialCapacity, float loadFactor) {
        ...
        table = new HMapIV.Entry[capacity];
        init();
    }
    ...
    public HMapIV() {
        ...
        table = new HMapIV.Entry[DEFAULT_INITIAL_CAPACITY];
        init();
    }
    ...
    public V get(int key) {
        int hash = hash(key);
        for (HMapIV.Entry<V> e = table[indexFor(hash, table.length)]; e != null; e = e.next) {
            int k;
            //System.out.println("Inside get, key: "+key);
            if (e.hash == hash && ((k = e.key) == key || key == k)) {
                //System.out.println("Inside get-1, key: "+key);            
                return e.value;
            }
        }
     ...
     */
    final HMapIV.Entry<V> getEntry(int key) {
        int hash = hash(key);
        for (HMapIV.Entry<V> e = table[indexFor(hash, table.length)]; e != null; e = e.next) {
            int k;
            if (e.hash == hash && ((k = e.key) == key || key == k)) {
    ...
    public V put(int key, V value) {
        int hash = hash(key);
        //System.out.println("Hash: " + hash + " key: " + key);
        int i = indexFor(hash, table.length);
        //System.out.println("Index: " + i);
        for (HMapIV.Entry<V> e = table[i]; e != null; e = e.next) {
            int k;
    ...
    private void putAllForCreate(MapIV<V> m) {
        for (Iterator<? extends MapIV.Entry<V>> i = m.entrySet().iterator(); i.hasNext();) {
    ...
    void resize(int newCapacity) {
        ...
        HMapIV.Entry<V>[] newTable = new HMapIV.Entry[newCapacity];
        transfer(newTable);
        table = newTable;
     ...
     */
    void transfer(HMapIV.Entry<V>[] newTable) {
        //System.out.println("Resize transfer");
        HMapIV.Entry<V>[] src = table;
        int newCapacity = newTable.length;
        for (int j = 0; j < src.length; j++) {
            HMapIV.Entry<V> e = src[j];
            if (e != null) {
                src[j] = null;
                do {
                    HMapIV.Entry<V> next = e.next;
    ...
    public V remove(int key) {
        HMapIV.Entry<V> e = removeEntryForKey(key);
        if (e != null) {
            return e.value;
        }
     ...
     */
    final HMapIV.Entry<V> removeEntryForKey(int key) {
        int hash = hash(key);
        int i = indexFor(hash, table.length);
        HMapIV.Entry<V> prev = table[i];
        HMapIV.Entry<V> e = prev;
     ...
     */
    final HMapIV.Entry<V> removeMapping(Object o) {
    ...
    public boolean containsValue(V value) {
        HMapIV.Entry<V>[] tab = table;
        for (int i = 0; i < tab.length; i++) {
            for (HMapIV.Entry<V> e = tab[i]; e != null; e = e.next) {
                if (value.equals(e)) {
                    return true;
                }
            }
    ...
    public Object clone() {
        ...
        result.table = new HMapIV.Entry[table.length];
    ...
    static class Entry<V> implements MapIV.Entry<V> {
         ...
         * Creates new entry.
         */
        Entry(int h, int k, V v, HMapIV.Entry<V> n) {
            value = v;
            next = n;
            key = k;
            hash = h;
    ...
    void addEntry(int hash, int key, V value, int bucketIndex) {
        HMapIV.Entry<V> e = table[bucketIndex];
        table[bucketIndex] = new HMapIV.Entry<V>(hash, key, value, e);
        if (size++ >= threshold) {
    ...
    private abstract class HashIterator<E> implements Iterator<E> {
        ...
        HMapIV.Entry<V> next; // next entry to return
        int expectedModCount; // For fast-fail
        int index; // current slot
        HMapIV.Entry<V> current; // current entry

        HashIterator() {
            expectedModCount = modCount;
            if (size > 0) { // advance to first entry
                HMapIV.Entry<V>[] t = table;
        ...
        final HMapIV.Entry<V> nextEntry() {
            if (modCount != expectedModCount) {
                throw new ConcurrentModificationException();
            }
            HMapIV.Entry<V> e = next;
            if (e == null) {
    ...
    public Collection<V> values() {
        ...
        return (vs != null ? vs : (values = new HMapIV.Values()));
    }
    ...
    private Set<MapIV.Entry<V>> entrySet0() {
        ...
        return es != null ? es : (entrySet = new HMapIV.EntrySet());
    }
    ...
    private final class EntrySet extends AbstractSet<MapIV.Entry<V>> {
        ...
        public boolean contains(Object o) {
            ...
            HMapIV.Entry<V> candidate = getEntry(e.getKey());
    ...
    private void readObject(ObjectInputStream s) throws IOException, ClassNotFoundException {
        ...
        table = new HMapIV.Entry[numBuckets];
    ...
    public MapIV.Entry<V>[] getEntriesSortedByValue() {
        ...
        MapIV.Entry<V>[] entries = new HMapIV.Entry[this.size()];
        int i = 0;
        HMapIV.Entry<V> next = null;
