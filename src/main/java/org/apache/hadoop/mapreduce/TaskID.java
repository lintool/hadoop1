...
/**
 * TaskID represents the immutable and unique identifier for a Map or Reduce
 * Task. Each TaskID encompasses multiple attempts made to execute the Map or
 * Reduce Task, each of which are uniquely indentified by their TaskAttemptID.
 *
 ...
 * representing whether the task is a map task or a reduce task. And the third
 * part is the task number. <br> An example TaskID is :
 * <code>task_200707121733_0003_m_000005</code> , which represents the fifth map
 * task in the third job running at the jobtracker started at
 * <code>200707121733</code>. <p> Applications should never construct or parse
 * TaskID strings , but rather use appropriate constructors or {@link #forName(String)}
 * method.
 *
 * @see JobID
 * @see TaskAttemptID
...
public class TaskID extends ID {
     ...
     * are defined as greater then maps.
     */
    @Override
