 ...
 * @author tim
 */
public class SequenceMemoryInputFormat extends InputFormat implements Callable {
    ...
    public SequenceMemoryInputFormat(ByteBuffer input) {
        // make a read only copy just in case
        this.buffer = input;
        this.buffer.rewind();
    }

    public SequenceMemoryInputFormat() {
        // make a read only copy just in case
    }
