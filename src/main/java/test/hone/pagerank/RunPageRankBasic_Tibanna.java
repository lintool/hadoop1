 ...
 *
 */
public class RunPageRankBasic_Tibanna {
...
//        // It's important to note that we don't add the PageRank mass to total... if PageRank mass
        ...
        public void cleanup(
            ...
            //System.out.println("Reducer done: " + taskId);
...
//            out.close();
    ...
    public static void main(String[] args) throws Exception {
        ...
        RunPageRankBasic_Tibanna thisJob = new RunPageRankBasic_Tibanna();
        System.exit(thisJob.run(args));
        //System.exit(res);
    }

    public RunPageRankBasic_Tibanna() {
    }

    private static int printUsage() {
        System.out.println("usage: [basePath] [numNodes] [start] [end] [numReducers]");
...
//    LOG.info(" - useRange?: " + useRange);
        ...
        // Iterate PageRank.
        for (int i = s; i < e; i++) {
            iteratePageRank(i, i + 1, basePath, n, useCombiner, useInmapCombiner, numReducers);
        }
    ...
    // Run each iteration.
    private void iteratePageRank(int i, int j, String basePath, int numNodes, boolean useCombiner, boolean useInMapperCombiner, int numReducers) throws Exception {
        // Each iteration consists of two phases (two MapReduce jobs).

        System.out.println("************** Iteration " + i + " ****************");
        double start = System.currentTimeMillis();
        // Job 1: distribute PageRank mass along outgoing edges.
        Object[] objs = phase1(i, j, basePath, numNodes, useCombiner, useInMapperCombiner, numReducers);
...
//        }
        ...
        System.out.println("Execution time for iteration" + i + ": " + (end - start) + "\n\n\n");
    }

    private Object[] phase1(int i, int j, String basePath, int numNodes, boolean useCombiner, boolean useInMapperCombiner, int numReducers) throws Exception {
        System.out.println("Phase 1 -- Iteration: " + i);

        Job job = new Job(new Configuration(), "PageRank:Basic:iteration" + j + ":Phase1");
        job.setMapperClass(RunPageRankBasic_Tibanna.MapClass.class);
        job.setReducerClass(RunPageRankBasic_Tibanna.ReduceClass.class);
