...
import org.apache.hadoop.util.map.HMapIV;
...
public class DocumentMapper extends Mapper<IntWritable, Document, PairOfInts, DoubleWritable> {
    ...
    private static int numberOfTopics = 5;
    ...
    private double likelihoodAlpha = 0;
    ...
    private Mapper<IntWritable, Document, PairOfInts, DoubleWritable>.Context context;
    private Job job;
    private DataOutputStream outputDocumentStream;
    private ByteArrayOutputStream_test baosDocument;
    private static int id = 0;

    public void setup(Mapper<IntWritable, Document, PairOfInts, DoubleWritable>.Context context) throws IOException {
        ...
        baosDocument = new ByteArrayOutputStream_test();
        outputDocumentStream = new DataOutputStream(new BufferedOutputStream(baosDocument));
        ...
        alpha = (double[]) context.getJobObject().getVariable("alphaVector");
        ...
        ArrayList<Object> betaList = job.getVariableList("reduce-beta");
        if (betaList.size() > 0) {
            //System.out.println("IN*********");
            beta = importBeta(betaList, numberOfTopics, numberOfTerms);
            job.remVariableList("reduce-beta");
        }

        if (beta == null) {
            System.out.println("Instantiate Beta");
            // System.exit(-1);
    ...
    @Override
    public void map(IntWritable key, Document value, Context context) throws IOException {
        ...
        VariationalInference_Tibanna.counter_total_doc.incrementAndGet();
        ...
        do {
            ...
            while (itr.hasNext()) {
                ...
                // System.out.println("Doc: " + key.get() + " Term:" + termID);
//                if (tempBeta == null) {
//                    System.out.println("Here temp-beta null, key: " + key.get() + " term: " + termID);
...
//
//                if (beta == null) {
//                    System.out.println("Here beta null, key: " + key.get() + " term: " + termID);
//                }

                likelihoodPhi += updatePhi(numberOfTopics, termCounts, tempBeta, tempGamma, phi, updateGamma);
            }
            ...
            if (Math.random() < 0.01) {
                ...
                //VariationalInference_Tibanna.counter_dummy_counter++;
            }
        ...
        //reporter.incrCounter(ParameterCounter.LOG_LIKELIHOOD, (long) (-documentLogLikelihood * Settings.DEFAULT_COUNTER_SCALE));
        synchronized (this) {
            VariationalInference_Tibanna.counter_log_likelihood += (long) (-documentLogLikelihood * Settings.DEFAULT_COUNTER_SCALE);
        }
        ...
        if (mapperCombiner) {
            ...
            if (learning) {
                if (Runtime.getRuntime().freeMemory() < VariationalInference_Tibanna.MEMORY_THRESHOLD) {
        ...
        if (!learning || !randomStartGamma) {
            //outputDocument = multipleOutputs.getCollector(Settings.GAMMA, Settings.GAMMA);
            value.setGamma(tempGamma);
            //outputDocument.collect(key, value);
            key.write(outputDocumentStream);
            value.write(outputDocumentStream);
        }

//        trainingTime = System.currentTimeMillis() - trainingTime;
//        //reporter.incrCounter(ParameterCounter.TRAINING_TIME, trainingTime);
//        VariationalInference_Tibanna.counter_training_time += trainingTime;
    }

    @Override
    public void close() throws IOException {
        //multipleOutputs.close();
        outputDocumentStream.flush();
        outputDocumentStream.close();
        baosDocument.flush();
        job.setVariable("map " + baosDocument.hashCode(), ByteBuffer.wrap(baosDocument.toByteArray()));
        baosDocument.close();
    ...
    /**
     * @param numberOfTopics number of topics defined by the current latent
     ...
     * @param phi the phi vector, take note that phi vector will be updated
     * accordingly.
     * @param phiSum a vector recording the sum of all the phi's over all the
     * terms, seeded from the caller program, take note that phiSum vector will
     * be updated accordingly.
     * @param phiWeightSum a vector recording the weighted sum of all the phi's
     * over all the terms, seeded from the caller program, take note that
     * phiWeightSum vector will be updated accordingly.
     * @param emptyPhiTable a boolean value indicates whether the phiSum and
     * phiWeightSum vector will be reset, they will be reset if True, and not.
     * otherwise
     * @param updateGamma the updated gamma vector, may or may not seeded from
     * the caller program, take note that updateGamma vector will be updated
     * accordingly
     * @return
     */
    public static double updatePhi(int numberOfTopics, int termCounts, double[] beta,
        ...
        if (beta == null) {
            ...
            System.out.println("numberOfTopics: " + numberOfTopics + " TermCounts: " + termCounts);
        }

        if (digammaGamma == null) {
    ...
    /**
     * Retrieve the beta array given the beta map and term index. If {@code beta}
     * is null or
     * {@code termID} was not found in {@code beta}, this method will pop a
     * message to
     * {@link System.out} and initialize it to avoid duplicate initialization in
     * the future.
     *
     * @param numberOfTopics number of topics defined by the current latent
     * Dirichlet allocation model.
     * @param beta a {@link HMapIV<double[]>} object stores the beta matrix, the
     * hash map is keyed by term index and valued by a corresponding double
     * array
     * @param termID term index
     * @param numberOfTerms size of vocabulary in the whole corpus, used to
     * initialize beta of the unloaded or non-initialized terms.
     * @return a double array of size {@link numberOfTopics} that stores the
     * beta value of term index in log scale.
     */
...
//            if (beta.get(termID) == null) {
//                for (double val : tempBeta) {
//                    System.out.println("TempBeta val: " + val + " term:" + termID);
//                    //System.exit(-1);
//                }
//            }
        }
...
//                    "Invalid beta vector for term " + pairOfIntFloat.getLeftElement() + "...");
            ...
            double normalizer = LogMath.add(pairOfIntFloat.getRightElement(),
                    VariationalInference_Tibanna.DEFAULT_ETA + Math.log(numberOfTerms));
            ...
            while (itr.hasNext()) {
                ...
                if (!beta.containsKey(termIndex)) {
                    ...
                    for (int i = 0; i < vector.length; i++) {
                        vector[i] = VariationalInference_Tibanna.DEFAULT_ETA;
                    }
     ...
     * @throws IOException
     */
    public static HMapIV<double[]> importBeta(ArrayList<Object> betaList,
            int numberOfTopics, int numberOfTerms) throws IOException {
        HMapIV<double[]> beta = new HMapIV<double[]>();
        ...
        //while (sequenceFileReader.next(pairOfIntFloat, hMapIFW)) {
        byte[] sequenceBytes;
        for (Object ele : betaList) {

            sequenceBytes = (byte[]) ele;
            dis = new DataInputStream(new ByteArrayInputStream(sequenceBytes));

            while (dis.available() > 0) {
                pairOfIntFloat.readFields(dis);
                hMapIFW.readFields(dis);

                //Preconditions.checkArgument(pairOfIntFloat.getLeftElement() > 0 && pairOfIntFloat.getLeftElement() <= numberOfTopics, "Invalid beta vector for term " + pairOfIntFloat.getLeftElement() + "...");
                ...
                while (itr.hasNext()) {
                    ...
                    //System.out.println("Here***** term: " + termIndex);
                    betaValue -= normalizer;
                    ...
                    if (!beta.containsKey(termIndex)) {
                        ...
                        vector[topicIndex] = LogMath.add(betaValue, vector[topicIndex]);
                        beta.put(termIndex, vector);
                        //System.out.println("Vector******: " + vector[topicIndex]);
                    } else {
                        beta.get(termIndex)[topicIndex] = LogMath.add(betaValue, beta.get(termIndex)[topicIndex]);
                    }
                }
